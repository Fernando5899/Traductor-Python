...
def hablar_texto(texto):
    """Mini-función aislada para que la tarjeta de audio no se trabe"""
    motor = pyttsx3.init()
...
def escuchar_y_detectar_local():
    ...
    # EL CICLO AHORA ABRAZA TODO DESDE AFUERA
    while True:
        # Prendemos y apagamos el micrófono en cada vuelta
        with sr.Microphone() as fuente:
            ...
            try:
                ...
                # Llamamos a nuestra mini-función segura
        ...
        # --- LA PAUSA PARA PREGUNTARTE ---
        # Como estamos fuera del 'with sr.Microphone()', el micro está apagado
        respuesta = input("\n¿Deseas traducir otra frase? (si/no): ").strip().lower()

        if respuesta not in ['si', 'sí', 's', 'yes', 'y']:
            print("\n🛑 Apagando el traductor. ¡Buen trabajo hoy!")
            break
